...
import { Zap, Plus, FileText, Receipt, TrendingUp, ChevronLeft, Printer, Settings, PenTool } from 'lucide-react';
...
const App = () => {
  ...
  if (v === 'landing') return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-10 text-center font-sans">
      ...
      <h1 className="text-4xl font-black mb-2 uppercase tracking-tighter">APA BizDesk 2.0</h1>
      <p className="text-slate-400 mb-10 text-sm">Professional White-Label Documents</p>
  ...
  if (v === 'onboarding' || v === 'settings') return (
    <div className="p-6 max-w-xl mx-auto pb-20 font-sans">
      ...
      <form className="space-y-4" onSubmit={(e:any)=>{e.preventDefault(); const d=Object.fromEntries(new FormData(e.target)); setP({...d, logo: p?.logo}); localStorage.setItem('ap_p', JSON.stringify({...d, logo: p?.logo})); setV('dashboard');}}>
        <div className="p-4 border-2 border-dashed rounded-2xl text-center relative bg-white h-24 flex flex-col justify-center">
          <p className="text-[8px] font-black text-slate-300">UPLOAD LOGO</p>
  ...
  if (v === 'create') return (
    <div className="p-6 max-w-xl mx-auto pb-20 font-sans bg-slate-50">
      <button onClick={() => setV('dashboard')} className="mb-4 text-slate-400 font-bold flex gap-2"><ChevronLeft/> Back</button>
      <h2 className="text-2xl font-black mb-6">Create {m}</h2>
      <div className="space-y-4">
        <input id="cl" placeholder="Customer Name" className="w-full p-4 border rounded-xl font-bold shadow-sm" />
        <div className="p-5 bg-white border rounded-3xl space-y-4 shadow-sm">
          ...
          <div className="flex gap-4">
            <input id="qt" type="number" defaultValue="1" className="w-1/4 bg-transparent border-b font-bold p-1" />
            <input id="pr" type="number" placeholder="Price" className="flex-1 bg-transparent border-b font-bold p-1" />
          </div>
        </div>
        <div className="p-4 bg-white border rounded-3xl space-y-2">
          <div className="flex justify-between items-center"><p className="text-[10px] font-black text-slate-400">SIGN MANUALLY (DRAW BELOW)</p><button onClick={()=>{const c=cv.current, x=c?.getContext('2d'); x?.clearRect(0,0,c?.width||0,c?.height||0)}} className="text-[10px] text-red-400 font-bold">CLEAR</button></div>
          <canvas ref={cv} width={300} height={120} onMouseDown={()=>setDr(true)} onMouseMove={draw} onMouseUp={()=>setDr(false)} onTouchStart={()=>setDr(true)} onTouchMove={draw} onTouchEnd={()=>setDr(false)} className="w-full h-28 border rounded-xl touch-none cursor-crosshair bg-slate-50" />
        </div>
        <button onClick={() => {
      ...
      <div className="max-w-xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex gap-1 overflow-x-auto">
            {LAYS.map(t => <button key={t} onClick={()=>setL(t)} className={`px-3 py-1.5 rounded-lg text-[8px] font-black transition ${l===t?'bg-slate-900 text-white':'bg-white border'}`}>{t}</button>)}
          </div>
        </div>
        <div id="doc" className={`bg-white p-8 shadow-2xl min-h-[750px] flex flex-col relative overflow-hidden ${l==='Modern'?'border-l-[15px] border-blue-600':l==='Bold'?'border-t-[25px] border-slate-900 bg-slate-50':l==='Minimal'?'border-none p-12':'border-t-[15px] border-slate-900'}`}>
          ...
          <div className="flex justify-between mb-10 border-b pb-6">
            <div>{p.logo && <img src={p.logo} className="h-10 mb-2 object-contain" />}<h2 className="font-black uppercase tracking-tight">{p.name}</h2><p className="text-[8px] text-slate-400 font-bold uppercase mt-1">{p.addr}<br/>{p.email}</p></div>
          ...
          <p className="text-[9px] font-black text-slate-300 uppercase mb-4">{ac.type==='Invoice'?'Bill To':'Received From'}: <span className="text-slate-900 text-lg ml-2">{ac.client}</span></p>
          <table className="w-full mb-10 text-left border-collapse">
            <thead><tr className="border-b-2 border-slate-900 text-[10px] font-black text-slate-400 uppercase"><th>Description</th><th className="text-right">Total</th></tr></thead>
          ...
          <div className="mt-auto flex justify-between items-end border-t-2 border-slate-900 pt-6">
            <div className="max-w-[220px]"><p className="text-[9px] font-black text-slate-300 uppercase mb-1">Info</p><p className="text-[9px] font-bold text-slate-500 whitespace-pre-wrap">{p.pay}</p>
            {ac.sig && <div className="mt-4"><img src={ac.sig} className="h-8 mb-1" /><div className="border-t w-16 opacity-20"></div><p className="text-[7px] font-black text-slate-300 uppercase">Signature</p></div>}</div>
            <div className="text-right"><p className="text-[10px] font-black text-slate-900 uppercase">Total {p.curr}{ac.total.toLocaleString()}</p></div>
          </div>
          <div className="mt-10 text-center border-t pt-4 text-[9px] font-black text-slate-900 uppercase italic opacity-30">{p.note || 'Thank you for your business!'}</div>
        </div>
        <button onClick={() => window.print()} className="w-full mt-6 py-5 bg-slate-900 text-white rounded-[2rem] font-black shadow-2xl flex justify-center gap-3 items-center"><Printer size={20}/> Print or Save PDF</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-6 pt-10 font-sans bg-white min-h-screen">
      <div className="flex justify-between items-center mb-10">
        <div className="flex items-center gap-3"><div className="bg-green-500 p-2.5 rounded-2xl text-white shadow-lg shadow-green-200"><Zap size={20} fill="currentColor"/></div>
      ...
      <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-sm">
        <h3 className="text-[10px] font-black text-slate-300 uppercase mb-6 tracking-widest">Recent activity</h3>
        ...
        {ds.length === 0 && <p className="text-center text-slate-300 py-4 font-bold text-[10px] uppercase tracking-widest">No history yet</p>}
      </div>
    </div>
  );
};
